...
irq_to_s5m8767_irq(struct sec_pmic_dev *s5m87xx, int irq)
{
...
{
	struct sec_pmic_dev *s5m87xx = irq_data_get_irq_chip_data(data);
...
static void s5m8767_irq_sync_unlock(struct irq_data *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(s5m87xx->irq_masks_cur); i++) {
		if (s5m87xx->irq_masks_cur[i] != s5m87xx->irq_masks_cache[i]) {
			...
			sec_reg_write(s5m87xx, S5M8767_REG_INT1M + i,
...
static inline struct s5m_irq_data *
irq_to_s5m8763_irq(struct sec_pmic_dev *s5m87xx, int irq)
{
...
static void s5m8763_irq_sync_unlock(struct irq_data *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(s5m87xx->irq_masks_cur); i++) {
		if (s5m87xx->irq_masks_cur[i] != s5m87xx->irq_masks_cache[i]) {
			s5m87xx->irq_masks_cache[i] = s5m87xx->irq_masks_cur[i];
			sec_reg_write(s5m87xx, S5M8763_REG_IRQM1 + i,
					s5m87xx->irq_masks_cur[i]);
		}
	}
...
{
	struct sec_pmic_dev *s5m87xx = data;
	u8 irq_reg[NUM_IRQ_REGS-1];
	int ret;
	int i;


	ret = sec_bulk_read(s5m87xx, S5M8767_REG_INT1,
...
static irqreturn_t s5m8763_irq_thread(int irq, void *data)
{
	...
	ret = sec_bulk_read(s5m87xx, S5M8763_REG_IRQ1,
...
int s5m_irq_resume(struct sec_pmic_dev *s5m87xx)
{
...
int sec_irq_init(struct sec_pmic_dev *s5m87xx)
{
	int i;
	int cur_irq;
	int ret = 0;
	...
	case S5M8763X:
		...
		sec_reg_write(s5m87xx, S5M8763_REG_STATUSM1, 0xff);
		sec_reg_write(s5m87xx, S5M8763_REG_STATUSM2, 0xff);
...
void sec_irq_exit(struct sec_pmic_dev *s5m87xx)
{
	if (s5m87xx->ono)
