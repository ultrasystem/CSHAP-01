...
static int s5m8767_reg_enable(struct regulator_dev *rdev)
{
	...
	return sec_reg_update(s5m8767->iodev, reg, pattern, mask);
}
...
static int s5m8767_reg_disable(struct regulator_dev *rdev)
{
	...
	return sec_reg_update(s5m8767->iodev, reg, (~pattern) | (ret <<6), mask);
}
				...
				int min_uV, int max_uV, unsigned *selector)
{
	...
	if (buck234_vol) {
		...
		//reg = ret;
		//printk("****************cym reg = %d\n", reg_buf);
		sec_reg_read(s5m8767->iodev, reg_buf, &val);
		//printk("****************cym1 reg = %d\n", reg_buf);
		val = (val & ~mask) | sel;
		//printk("reg_id = %d, reg_bug = %d, val = 0x%x\n", reg_id, reg_buf, val);
		ret = sec_reg_write(s5m8767->iodev, ret, val);
	}
...
static __devinit int s5m8767_pmic_probe(struct platform_device *pdev)
{
	...
	val &= 0x8;
	val |= 0x2;
	sec_reg_write(s5m8767->iodev, reg, val);
#endif
...
#if 0
	{
		...
		{
			sec_reg_read(s5m8767->iodev, i, &val);
